...
module.exports = function (html, callback) {
  ...
  if (html) {
    ...
    if (foundAnything(errors)) {
      ...
      if (answers.length === 2) {
          // Whole city CEP
          location = answers.eq(0).text()
          if (unexpectedLocation(location)) err = new UnexpectedLocation(location)
          else address = getAddress(answers, false);

      } else if (answers.length < 4) err = new UnexpectedAnswerAmount(answers.length)
      else {

        location = answers.eq(2).text()
        if (unexpectedLocation(location)) err = new UnexpectedLocation(location)
        else address = getAddress(answers, true)
...
function getAddress (answers, full) {
  return full ? {
    ...
    , uf:         answers.eq(2).text().trim().split('/')[1].trim()
      ...
      logradouro: ''
    , bairro:     ''
    , localidade: answers.eq(0).text().trim().split('/')[0].trim()
    , uf: answers.eq(0).text().trim().split('/')[1].trim()
  }
}
